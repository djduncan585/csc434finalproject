...
source = open("../../future-shock.txt", encoding="utf-8").read().lower()
filteredtextlist = []
print(len(source))
for i in range(len(source)):
    if source[i] in char_indices.keys():
        filteredtextlist.append(source[i])
filteredtext = ''.join(filteredtextlist)
filewrite = open("future-shock-filtered.txt", "w")
filewrite.write(filteredtext)
filewrite.close()